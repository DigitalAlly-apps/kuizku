...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    catch (error) { return fail(error instanceof DOMException && error.name === "AbortError" ? "gemini_timeout" : "gemini_unavailable", 503); }
    if (!response.ok) {
    ...
    } catch { return fail("gemini_invalid_response", 502, { model }); }

    if (grades.length !== expectedIds.length || new Set(grades.map((grade) => grade.questionId)).size !== expectedIds.length || grades.some((grade) => !expectedIds.includes(grade.questionId) || !Number.isFinite(grade.suggestedScore))) return fail("gemini_invalid_schema", 502, { model });
