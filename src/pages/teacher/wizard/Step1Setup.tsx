...
export default function Step1Setup({ initial, onNext, submitLabel = 'Lanjut: Pilih Format →', heading = 'Pengaturan Ujian', intro = 'Isi informasi dasar dan tipe kegiatan ini.' }: Props) {
  ...
  const personalRosterMode = personalMode && useSavedRoster;
  ...
  const validate = () => {
    ...
    if (personalRosterMode && !className) e.className = 'Pilih grup murid terlebih dahulu';
    if (personalRosterMode && selectedPersonalGroup && !students.some(student => student.groupId === selectedPersonalGroup.id)) e.className = 'Grup ini belum memiliki murid.';
  ...
  const handleNext = async () => {
    ...
    if (personalRosterMode && !subjects.some(item => item.name.trim().toLocaleLowerCase() === subject.trim().toLocaleLowerCase())) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
        ...
        {personalMode && <div className="form-group"><label className="form-label">Peserta ujian</label><div className="student-access-choice" role="radiogroup" aria-label="Cara memilih peserta"><button type="button" role="radio" aria-checked={!useSavedRoster} className={!useSavedRoster ? 'is-active' : ''} onClick={() => setUseSavedRoster(false)}><strong>Mode biasa</strong><span>Ketik mapel, kelas, dan daftar peserta bebas seperti sebelumnya.</span></button><button type="button" role="radio" aria-checked={useSavedRoster} className={useSavedRoster ? 'is-active' : ''} onClick={() => setUseSavedRoster(true)}><strong>Pakai daftar murid</strong><span>Pilih grup tersimpan; murid memilih nama dari dropdown.</span></button></div></div>}
        <div className="form-row form-row-2">
          <div className="form-group"><label className="form-label" htmlFor="s1-subject">Mata Pelajaran <span style={{ color: 'var(--danger)' }}>*</span></label><input id="s1-subject" list={personalRosterMode ? 'personal-subject-suggestions' : undefined} className={`form-input ${errors.subject ? 'error' : ''}`} placeholder="Contoh: Fiqh, Kuis Peserta Lomba..." value={subject} onChange={e => { setSubject(e.target.value); setErrors(er => ({ ...er, subject: '' })); }} />{personalRosterMode && <datalist id="personal-subject-suggestions">{subjects.map(item => <option key={item.id} value={item.name} />)}</datalist>}{personalRosterMode && <span className="form-hint">Ketik mapel/kategori baru atau pilih yang pernah dipakai.</span>}{errors.subject && <span className="form-error">{errors.subject}</span>}</div>
          <div className="form-group"><label className="form-label" htmlFor="s1-class">{personalRosterMode ? 'Grup murid' : 'Kelas (opsional)'}</label>{personalRosterMode ? <select id="s1-class" className={`form-select ${errors.className ? 'error' : ''}`} value={selectedPersonalGroup?.id ?? ''} onChange={e => { setClassName(e.target.value); setErrors(er => ({ ...er, className: '' })); }}><option value="">Pilih grup</option>{groups.map(item => <option key={item.id} value={item.id}>{item.name} ({students.filter(student => student.groupId === item.id).length} murid)</option>)}</select> : <input id="s1-class" className="form-input" placeholder="Contoh: Kelas 10A, Kelas 12 IPA..." value={className} onChange={e => setClassName(e.target.value)} />}{errors.className && <span className="form-error">{errors.className}</span>}</div>
        </div>
        {!personalRosterMode && <div className="form-group">
        ...
        <section className="card" style={{ padding: 'var(--sp-4)' }}>
          ...
          <div className="form-group"><label className="form-label" htmlFor="s1-key-release">Review Jawaban Murid</label><select id="s1-key-release" className="form-select" value={settings.answerKeyReleaseMode ?? 'NEVER'} onChange={e => setSetting('answerKeyReleaseMode', e.target.value as ExamSettings['answerKeyReleaseMode'])}><option value="IMMEDIATE">Langsung</option><option value="AFTER_EXAM_END">Setelah ujian berakhir</option><option value="NEVER">Jangan tampilkan</option></select><span className="form-hint">Murid dapat melihat jawabannya dan status benar atau salah sesuai waktu rilis yang dipilih.</span></div>
