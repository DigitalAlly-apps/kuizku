    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
        ...
        <div className="form-row form-row-2">
          ...
          <div className="form-group"><label className="form-label" htmlFor="s1-class">{personalRosterMode ? 'Grup murid' : 'Kelas (opsional)'}</label>{personalRosterMode ? <select id="s1-class" className={`form-select ${errors.className ? 'error' : ''}`} value={selectedPersonalGroup?.id ?? ''} onChange={e => { setClassName(e.target.value); setErrors(er => ({ ...er, className: '' })); }}><option value="">Pilih grup</option>{groups.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : <input id="s1-class" className="form-input" placeholder="Contoh: Kelas 10A, Kelas 12 IPA..." value={className} onChange={e => setClassName(e.target.value)} />}{errors.className && <span className="form-error">{errors.className}</span>}</div>
        </div>
