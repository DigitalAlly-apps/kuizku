...
const mapStudent = (row: any): PersonalExamStudent => ({ id: row.id, teacherId: row.teacher_id, groupId: row.group_id, name: row.name });
...
export const personalExamApi = {
  ...
  async load(teacherId: string) {
    const [subjects, groups, students] = await Promise.all([
      ...
      supabase.from('personal_exam_students').select('*').eq('teacher_id', teacherId).order('name'),
    ]);
