...
export function PersonalDataModal({ onClose }: { onClose: () => void }) {
  const { subjects, groups, students, addSubject, addGroup, addStudent, addStudents, remove } = usePersonalExam();
  ...
  const savePastedStudents = async () => {
    ...
    setPastedStudents(''); setError(''); setNotice(`${names.length} murid berhasil dimasukkan ke grup ${groups.find(group => group.id === studentGroup)?.name ?? ''}.`);
  };
